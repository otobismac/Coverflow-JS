	...
	$.fn.coverflow = function(options) {
		...
		$.extend(this, {
			...
			last: function() {
				...
				ind = this.numCovers - 1;
			},
			getIndex: function(){
				console.log(this.currentIndex);
				return indexExt = this.currentIndex;
			},
		...
		var api = {};
		var fns = ["next", "prev", "first", "last", "toIndex", "getIndex"];
...
//CUSTOM SCRIPT FOR COVERFLOW
var item = 0;
function adjustTextCenter(factor){
	$(".cardText").css( { marginLeft :  factor } );
}
function changeTitle(){
	displayCardText(getCardsSliderIndex());
}
function displayCardText(cardId){
	$(".cardText").hide();
	$("#card" + cardId).show();
}
window.coverflow = $("#coverflow").coverflow({
	currentIndex: 4,
		allowSwipe: true,
		allowClick: true,
		changeFn: changeTitle,
	backgroundColor: "#FFF"
});
$( window ).resize(function() {
	console.log( "Handler for size() called: " + $( window ).width() );
	adjustTextCenter($( window ).width());
});
